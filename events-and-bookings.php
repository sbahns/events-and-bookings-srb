 ...
 * Description: Fork of WPMU DEV Events+ - Manage events and bookings with ease.
 * Version: 2.0
 ...
 */
 ...
 class Eab_EventsHub {
     ...
     function user_has_cap( $allcaps, $caps = null, $args = null ) {
         ...
         // Check if $caps is provided and is an array
         if ( ! is_array( $caps ) ) {
             return $allcaps;
         }
     
         // Check if the current user has capabilities related to events
         if ( preg_grep( '/(_event|_events)/i', $caps ) ) {
             // Check if user is administrator
             if ( in_array( 'administrator', $current_user->roles ) ) {
                 foreach ( $caps as $cap ) {
                     $allcaps[$cap] = true; // Set capability to true
                 }
                 return $allcaps;
             }
             ...
             foreach ( $caps as $cap ) {
                 $capable = false; // Initialize capability check
                 ...
                 switch ( $cap ) {
                     case 'read_events':
                         ...
                         break;
                     default:
                         // Check if user can perform similar capability on a post
                 ...
                 // Apply filter to determine final capability
                 $capable = apply_filters( 'eab-capabilities-user_can', $capable, $cap, $current_user, $args );
     
                 // If capable, set the capability in $allcaps array
                 if ( $capable ) {
                     $allcaps[$cap] = true;
                 }
             }
